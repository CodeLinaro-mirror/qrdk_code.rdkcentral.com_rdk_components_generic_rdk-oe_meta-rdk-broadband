...
DEPENDS:append = " ${@bb.utils.contains_any('DISTRO_FEATURES', 'cellular_libqmi_support', 'libqmi', '', d)}"
...
inherit autotools pkgconfig systemd ${@bb.utils.contains_any("DISTRO_FEATURES", "kirkstone wrynose", "python3native", "pythonnative", d)} 
...
CFLAGS:append = " \
    ...
    ${@bb.utils.contains_any('DISTRO_FEATURES', 'cellular_libqmi_support', ' -I${STAGING_INCDIR}/libqmi-glib', '', d)} \
...
do_compile:prepend () {
    ...
    if ${@bb.utils.contains_any('DISTRO_FEATURES', 'WanManagerUnificationEnable', 'true', 'false', d)}; then
...
V_BINARY_FILE .= "${@bb.utils.contains_any('DISTRO_FEATURES', 'cellular_mgr_lite', 'lite', '', d)}"
V_SERVICE_FILE .= "${@bb.utils.contains_any('DISTRO_FEATURES', 'cellular_mgr_lite', 'Lite', '', d)}"
...
ENABLE_CELLULAR_MGR_LITE = "--enable-cellularmgrlite=${@bb.utils.contains_any('DISTRO_FEATURES', 'cellular_mgr_lite', 'yes', 'no', d)}"
EXTRA_OECONF:append  = " ${ENABLE_CELLULAR_MGR_LITE}"
EXTRA_OECONF:append  = " --with-ccsp-platform=bcm --with-ccsp-arch=arm "

ENABLE_CELLULAR_MGR_LTE_USB = "--enable-lteusbsupport=${@bb.utils.contains_any('DISTRO_FEATURES', 'lte_usb_support', 'yes', 'no', d)}"
EXTRA_OECONF:append  = " ${ENABLE_CELLULAR_MGR_LTE_USB}"
