...
DEPENDS:append = " ${@bb.utils.contains('DISTRO_FEATURES', 'enable_rdkscheduler', ' trower-base64 msgpack-c rdk-scheduler cimplog', " ", d)}"
...
CFLAGS:append += "${@bb.utils.contains('DISTRO_FEATURES', 'enable_rdkscheduler',' -I${STAGING_INCDIR}/trower-base64 -I${STAGING_INCDIR}/msgpackc -I${STAGING_INCDIR}/cimplog','',d)}"
...
CFLAGS:append += "${@bb.utils.contains('DISTRO_FEATURES', 'thermalctrl','-DFAN_THERMAL_CTR','',d)}"
LDFLAGS:append += "${@bb.utils.contains('DISTRO_FEATURES', 'thermalctrl',' -lhal_platform','',d)}"
...
CFLAGS:append += "${@bb.utils.contains('DISTRO_FEATURES', 'warehouseFan','-DLIMITED_FAN_WAREHOUSE','',d)}"
LDFLAGS:append += "${@bb.utils.contains('DISTRO_FEATURES', 'warehouseFan',' -lhal_platform','',d)}"
...
EXTRA_OECONF:append = "${@bb.utils.contains('DISTRO_FEATURES', 'enable_device_prioritization',' --enable-device_prioritization','',d)}"
EXTRA_OECONF:append = "${@bb.utils.contains('DISTRO_FEATURES', 'enable_rdkscheduler',' --enable-rdk_scheduler','',d)}"
LDFLAGS:append = "${@bb.utils.contains('DISTRO_FEATURES', 'enable_rdkscheduler',' -lcimplog','',d)}"
